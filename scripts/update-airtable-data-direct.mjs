...
async function updateMinistries() {
  ...
  try {
    const allMinistries = await getAllRecords('Ministries');

    for (const [ministryName, updates] of Object.entries(ministryUpdates)) {
      const ministry = allMinistries.find(m =>
        m["Ministry Name"] === ministryName ||
        m["Ministry Name"] === "Men's Ministry (CFBC Fraternity)" && ministryName === "Men's Fraternity"
      );

      if (ministry) {
        console.log(`Updating: ${ministryName}`);
        await base('Ministries').update(ministry.id, updates);
    ...
    if (!childrenChurch) {
      ...
      await base('Ministries').create([{ fields: newChildrensChurch }]);
...
async function updateMissions() {
  ...
  try {
    const allMissions = await getAllRecords('Missions');

    for (const [missionaryName, updates] of Object.entries(missionUpdates)) {
      const mission = allMissions.find(m =>
        m["Missionary Name"] === missionaryName ||
        m["Missionary Name"] === "The Friends of Israel Gospel Ministry" && missionaryName === "Jeff & Arlene Berg"
      );

      if (mission) {
        console.log(`Updating: ${missionaryName}`);
        // If we're renaming, include the name in the update
        if (mission["Missionary Name"] === "The Friends of Israel Gospel Ministry") {
          updates["Missionary Name"] = "Jeff & Arlene Berg";
        }
        await base('Missions').update(mission.id, updates);
    ...
    if (!shortTerm) {
      ...
      await base('Missions').create([{ fields: newShortTermMissions }]);
